export const CRYPTOS = [
  'BTC', 'ETH', 'SOL', 'BNB', 'XRP', 'ADA', 'AVAX', 'DOGE', 'DOT', 'MATIC',
  'LINK', 'UNI', 'ATOM', 'NEAR', 'APT', 'ARB', 'OP', 'SUI', 'SEI', 'INJ',
  'TIA', 'JUP', 'WIF', 'PEPE', 'BONK', 'RENDER', 'FET', 'TAO', 'WLD', 'TON',
  'HBAR', 'FIL', 'LTC', 'BCH', 'ETC', 'SAND', 'MANA', 'AXS', 'GALA', 'IMX'
]

export const TIMEFRAMES = ['1m', '3m', '5m', '15m', '30m', '1h', '2h', '4h', '8h', '12h', '1D', '1W', '1M']

export const LEVERAGES = [2, 3, 5, 10, 20, 25, 50, 75, 100, 125, 150, 200]

export const RISK_PRESETS = [0.5, 1, 1.5, 2, 2.5, 3, 5, 7, 10]

export const RESULTS = {
  WIN: { label: 'Ganada', color: 'green', emoji: '✓' },
  LOSS: { label: 'Perdida', color: 'red', emoji: '✗' },
  BE: { label: 'Breakeven', color: 'yellow', emoji: '≈' }
}

export const MONTHS_ES = [
  'Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio',
  'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre'
]

export const MONTHS_SHORT = [
  'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun',
  'Jul', 'Ago', 'Sep', 'Oct', 'Nov', 'Dic'
]

export const DAYS_ES = ['Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb', 'Dom']

export const VIEWS = {
  DASHBOARD: 'dashboard',
  CALENDAR: 'calendar',
  STATS: 'stats',
  ANNUAL: 'annual',
  SETTINGS: 'settings'
}

export const LS_KEYS = {
  TRADES: 'cj_trades',
  CAPS: 'cj_caps',
  API_KEY: 'cj_apiKey',
  SETTINGS: 'cj_settings'
}
